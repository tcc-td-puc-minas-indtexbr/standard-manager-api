...
class StandardManagerServiceTestCase(BaseFunctionalTestCase):
    ...
    def setUpClass(cls):
        ...
        if cls.EXECUTE_FIXTURE:
            ...
            file_name = ROOT_DIR + '/tests/datasets/nosql/create.table.standard-manager-dynamodb-StandardTable.json'
            DynamoDBHelper.create_table(connection=get_connection(),
                                        table_name=table_name,
                                        file_name=file_name)
            logger.info(f'table {table_name} created')

            file_name = ROOT_DIR + '/tests/datasets/seeders/seeder.standard-manager-dynamodb-StandardTable.json'
